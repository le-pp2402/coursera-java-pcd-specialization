...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        final int worker = tasks;
        final Phaser[] phasers = new Phaser[worker];
        for (int i = 0; i < worker; i++) {
            phasers[i] = new Phaser(1);
        }

        final int chunkSize = (n + worker - 1) / worker;
        ForkJoinPool pool = ForkJoinPool.commonPool();
        java.util.concurrent.Future<?>[] futures = new java.util.concurrent.Future[worker];

        for (int t = 0; t < worker; t++) {
            final int tid = t;

            futures[tid] = pool.submit(() -> {
                final int left = tid * chunkSize + 1;
                final int right = Math.min(n, left + chunkSize - 1);
                ...
                for (int it = 0; it < iterations; it++) {
                    ...
                    next[left] = (curr[left - 1] + curr[left + 1]) / 2.0;
                    if (right > left) {
                        next[right] = (curr[right - 1] + curr[right + 1]) / 2.0;
                    }

                    int id = phasers[tid].arrive();
                    ...
                    if (tid > 0) {
                        phasers[tid - 1].awaitAdvance(id);
                    }
                    if (tid < tasks - 1) {
                        phasers[tid + 1].awaitAdvance(id);
                    }

                    final double[] tmp = curr;
                    curr = next;
                    next = tmp;
                }
            });
