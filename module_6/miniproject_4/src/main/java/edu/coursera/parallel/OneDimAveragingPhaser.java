...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int t = 0; t < tasks; t++) {
            ...
            th[tid] = new Thread(() -> {
                final int chunkSize = (n + tasks - 1) / tasks;
                final int lhs = (tid * chunkSize) + 1;
                int rhs = (lhs + chunkSize) - 1;
                if (rhs > n) {
                    rhs = n;
                }

                double[] currentNew = myNew;
                double[] currentVal = myVal;

                for (int it = 0; it < iterations; ++it) {
                    currentNew[lhs] = (lhs == 0 ? currentVal[lhs] : (currentVal[lhs - 1] + currentVal[lhs + 1]) / 2.0);
                    currentNew[rhs] = (rhs == n - 1 ? currentVal[rhs]
                            : (currentVal[rhs - 1] + currentVal[rhs + 1]) / 2.0);

                    int phase = phasers[tid].arrive();

                    for (int iid = lhs + 1; iid + 1 <= rhs; ++iid) {
                        currentNew[iid] = (currentVal[iid - 1] + currentVal[iid + 1]) / 2.0;
                    }

                    if (tid - 1 >= 0)
                        phasers[tid - 1].awaitAdvance(phase);
                    if (tid + 1 < tasks)
                        phasers[tid + 1].awaitAdvance(phase);

                    double[] tmp = currentNew;
                    currentNew = currentVal;
                    currentVal = tmp;
                }
            });
