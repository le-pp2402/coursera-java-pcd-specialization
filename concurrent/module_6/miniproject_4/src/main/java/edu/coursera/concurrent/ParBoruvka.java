...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        AtomicInteger connectCpn = nodesLoaded.size();
        List<Lock> locks = new ArrayList<ReentrantLock>(connectCpn);
        while (connectCpn != 1) {
            ParComponent node = nodesLoaded.poll();
            if (node == null)
                continue;
            if (locks.get(node.nodeId).tryLock()) {
                // TODO: implement DSU & priority queue
            } else {
